...
PIXEL_RGBA HsvToRgb(double h, double s, double v, uint8_t a = 255) {
    ...
    if (s == 0) {
        ...
        int32_t i = static_cast<int32_t>(h / 60.0);
        double f = (h / 60.0) - i;
        double p = v * (1.0 - s);
        double q = v * (1.0 - s * f);
...
inline void BlendPixel(PIXEL_RGBA* buf, int32_t w, int32_t h, int32_t x, int32_t y, PIXEL_RGBA c) {
    ...
    if (c.a == 255) {
        ...
        buf[idx].r = static_cast<uint8_t>(c.r * alpha + bg.r * invAlpha);
        buf[idx].g = static_cast<uint8_t>(c.g * alpha + bg.g * invAlpha);
        buf[idx].b = static_cast<uint8_t>(c.b * alpha + bg.b * invAlpha);
        buf[idx].a = static_cast<uint8_t>((std::min)(255, bg.a + c.a));
    }
}
...
void DrawBox(PIXEL_RGBA* buf, int32_t w, int32_t h, int32_t x, int32_t y, int32_t rw, int32_t rh, PIXEL_RGBA fillColor, PIXEL_RGBA borderColor, int32_t borderThick, int32_t radius, bool gradient) {
    ...
    for (int32_t iy = sy; iy < ey; ++iy) {
        ...
        for (int32_t ix = sx; ix < ex; ++ix) {
            ...
            bool isBorder = false;
            if (borderThick > 0)
                if (lx < borderThick || lx >= rw - borderThick || ly < borderThick || ly >= rh - borderThick) isBorder = true;
...
void DrawRing(PIXEL_RGBA* buf, int32_t w, int32_t h, int32_t cx, int32_t cy, float radius, float thickness, PIXEL_RGBA col) {
    int32_t rOut = static_cast<int32_t>(ceil(radius));
    int32_t rIn = static_cast<int32_t>(radius - thickness);
    if (rIn < 0) rIn = 0;
...
bool func_proc_video_midi_visualizer(FILTER_PROC_VIDEO* video) {
    ...
    int32_t w = static_cast<int32_t>(track_width.value);
    int32_t h = static_cast<int32_t>(track_height.value);
    ...
    if (check_auto_fit.value) {
        ...
        if (range < 1) range = 1;
        if (scrollMode == 0 || scrollMode == 1) keySize = static_cast<double>(h) / range;
        else keySize = static_cast<double>(w) / range;
    }
    int32_t filterCh = static_cast<int32_t>(track_ch_target.value);
    int32_t minVel = static_cast<int32_t>(track_vel_min.value);
    ...
    int32_t radius = static_cast<int32_t>(track_radius.value);
    bool gradient = check_gradient.value;
    int32_t pad = static_cast<int32_t>(track_padding.value);
    bool border = check_border.value;
    int32_t borderW = static_cast<int32_t>(track_border_w.value);
    PIXEL_RGBA borderColor = { color_border.value.r, color_border.value.g, color_border.value.b, 255 };
    uint8_t noteAlpha = static_cast<uint8_t>((track_note_alpha.value / 100.0) * UINT8_MAX);
    ...
    uint8_t ledAlpha = static_cast<uint8_t>((track_kb_led_alpha.value / 100.0) * UINT8_MAX);
    int32_t grid_width = static_cast<int32_t>(track_grid_width.value);
    int32_t range = maxKey - minKey + 1;
    int32_t sinkDepth = static_cast<int32_t>(track_sink_depth.value);
    double rippleMaxR = track_ripple_size.value;
    double rippleLife = track_ripple_time.value;
    int32_t particleAmt = static_cast<int32_t>(track_particle_amt.value);
    ...
    int64_t lookBackTicks = static_cast<int64_t>(maxLookBackSec * ticksPerSec);
    ...
    for (const auto& note : data.notes) {
        ...
        if ((enableRipple || enableParticle) && reactionMode > 0) {
            int64_t elapsedTick = currentTick - note.startTick;
            if (elapsedTick >= 0 && elapsedTick < lookBackTicks) {
                EffectEvent ev;
                ev.pitch = note.pitch;
                ev.timeSec = elapsedTick / ticksPerSec;
    ...
    if (enableParticle && reactionMode > 0) {
        for (const auto& note : data.notes) {
            ...
            int64_t elapsedTick = currentTick - note.startTick;
            if (elapsedTick >= 0 && elapsedTick < static_cast<int64_t>(particleLife * ticksPerSec)) {
                uint32_t seed = note.pitch * 12345 + note.channel * 678 + note.startTick;
                particleEvents.push_back({ note.pitch, elapsedTick / ticksPerSec, GetNoteColor(note), seed });
            }
        }
    }
    ...
    auto DrawLED = [&](int32_t kx, int32_t ky, int32_t kw, int32_t kh, PIXEL_RGBA col) {
        ...
        if (scrollMode == 0 || scrollMode == 1) {
            ledSize = static_cast<int32_t>(kh * ledSizeRatio);
            if (ledSize < 2) ledSize = 2;
            if (scrollMode == 0) {
                int32_t distFromBase = static_cast<int32_t>(kw * (1.0 - ledPosRatio));
                lx = kx + distFromBase;
            } else {
                int32_t distFromBase = static_cast<int32_t>(kw * (1.0 - ledPosRatio));
            ...
            ledSize = static_cast<int32_t>(kw * ledSizeRatio);
            if (ledSize < 2) ledSize = 2;
            lx = kx + (kw - ledSize) / 2;
            if (scrollMode == 2) {
                int32_t distFromBase = static_cast<int32_t>(kh * (1.0 - ledPosRatio));
    ...
    auto RenderRipple = [&](int32_t cx, int32_t cy, PIXEL_RGBA col, double timeSec) {
        ...
        col.a = static_cast<uint8_t>(col.a * alphaFactor);
        float thick = 8.0f + 12.0f * static_cast<float>(progress);
        int32_t rOut = static_cast<int32_t>(ceil(radius + thick));
        int32_t bx0 = cx - rOut;
        int32_t by0 = cy - rOut;
        int32_t bx1 = cx + rOut;
        int32_t by1 = cy + rOut;
    ...
    if (check_grid_v.value && tpqn > 0) {
        ...
        if (pixelsPerBeat > 0.5) {
            int64_t tickOffset = static_cast<int64_t>(maxDim / zoomT) + static_cast<int64_t>(scrollPos / zoomT) + tpqn * 8;
            int64_t minTick = (std::max)(0ll, currentTick - tickOffset);
    ...
    if (enableParticle && reactionMode > 0) {
        ...
        for (const auto& note : data.notes) {
            ...
            if (note.pitch < minKey || note.pitch > maxKey) continue;
            if (note.velocity < minVel) continue;
            double noteDurationSec = static_cast<double>(note.endTick - note.startTick) / ticksPerSec;
            double timeSinceStart = static_cast<double>(currentTick - static_cast<int64_t>(note.startTick)) / ticksPerSec;
            ...
            int32_t k_min = static_cast<int32_t>(ceil(minEmitTime / emissionInterval));
            int32_t k_max = static_cast<int32_t>(floor(maxEmitTime / emissionInterval));
            ...
            int32_t countPerStep = (std::max)(1, static_cast<int32_t>(particleAmt * 0.2));
            ...
            params.k_min = k_min;
            params.emissionInterval = static_cast<float>(emissionInterval);
            params.timeSinceStart = static_cast<float>(timeSinceStart);
            params.baseSeed = note.pitch * 99999 + note.startTick;
            params.cx = static_cast<float>(cx);
            params.cy = static_cast<float>(cy);
            params.scrollMode = scrollMode;
            params.gravity = 500.0f;
            params.particleLife = static_cast<float>(particleLife);
            for (int32_t idx = 0; idx < total_particles; idx += 8) {
                ...
                Avx2Utils::BlendPointsAVX2(imgBuf.data(), w, h, px, py, ages, block, noteColor, static_cast<float>(particleLife));
            }
        }
    }
