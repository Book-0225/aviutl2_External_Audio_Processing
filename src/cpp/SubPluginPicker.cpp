...
static constexpr const wchar_t* APP_CLASS = L"SubPluginPickerWindowClass";
...
struct PickerState {
    const std::vector<IAudioPluginHost::SubPluginInfo>* candidates = nullptr;
...
LRESULT CALLBACK PickerWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            CreateWindowExW(0, L"STATIC", L"読み込むサブプラグインを選択してください", WS_CHILD | WS_VISIBLE, 12, 10, width - 24, 20, hWnd, nullptr, cs->hInstance, nullptr);
            ...
            if (state->candidates) {
                for (const auto& c : *state->candidates) {
                    std::wstring line = StringUtils::Utf8ToWide(c.name);
                    if (!c.category.empty()) line += L"  [" + StringUtils::Utf8ToWide(c.category) + L"]";
                    SendMessage(state->listBox, LB_ADDSTRING, 0, reinterpret_cast<LPARAM>(line.c_str()));
                }
...
std::string ShowSubPluginPicker(HINSTANCE hInstance, HWND parentWindow, const std::wstring& dialogTitle, const std::vector<IAudioPluginHost::SubPluginInfo>& candidates) {
    ...
    if (state.confirmed && state.selectedIndex >= 0 &&
        ...
        return candidates[state.selectedIndex].id;
    }
    return "";
}
